...
**Status:** Shipping & self-hosting (**v0.9.30** — [`VERSION`](VERSION) is the source of truth; this
...
| ✅ | **The agent verify records but never tries** — the verdict cache had one reader and two writers: the agent boundary *recorded* a verdict for a tree it had just verified and never *consulted* one, so every completing tasklist paid a full suite run the cache could already answer. The boundary now tries the cache before it runs the hook, and `test/verify-cache.sh` drives `engine/agent.sh` directly and asserts on how many times the hook itself executed — reproducing the miss against a neutered engine first, so the test cannot pass by restating behaviour that always worked. That reader honoured `status=0` only — a limit of the first implementation and not a design choice, and `chief/120-a-red-verdict-is-re-earned` removed it; the cache's contract, key component by key component, is [`docs/reference/verify-hook.md`](docs/reference/verify-hook.md) | `chief/117-the-agent-verify-records-but-never-tries` · `2e79e65` |
| ✅ | **A document can claim something downstream** — `downstreamCounterpart` connects a tasklist to the tasklist that completes it; nothing connected a **document** to the downstream tree it asserts something about, and the obligation runs the wrong way for the downstream repo to fix it. `.chief/claims.json` now declares `{document, claim, repo, path}`, resolved through the same `crossrepo.sh` lookup `dependsOn` uses, over a deliberately tiny vocabulary of two predicates (`present` · `absent`) — a claim that cannot be reduced to a predicate stays prose and stays invisible, exactly as an undeclared counterpart does. Reports, never fails (`engine/claims.sh`, `test/doc-claims.sh`). Measured: koine's encoding-gate document said three tests had no encoding; agora encoded all three **49 minutes later**, and koine carried the claim into two authored tasklists a week on | `chief/118-a-document-can-claim-something-downstream` · `678ac94` |
| ✅ | **The budget counts the cheap resource** — the host-wide budget governed AGENT TURNS against a ceiling derived from CPU cores, counted gates without ever consulting the count, and read load average only to print a word — so a box could be measurably oversubscribed while the scheduler believed two thirds of its capacity was free. Agent turns and gates are now separate budgets because they cost different things, the load ceiling is an input to admission rather than decoration, and the display reports the ADMISSION answer instead of arithmetic: a control that is currently refusing reports 0 for both budgets and names itself. Measured 2026-09-03 on a 14-core host: `5 live run(s) · 5 agent turn(s) · 0 gate(s) · load average: 14.32 / 14 physical core(s) · OVERSUBSCRIBED` — every number true, two of the three controls invisible, and the launch loop would have admitted **nine more runs** ([`docs/reference/concurrency.md`](docs/reference/concurrency.md)) | `chief/121-the-budget-counts-the-cheap-resource` · `57d0e4b` |

### Maintenance paydown — ✅ shipped (`900`–`901`, 2026-09-03 → 09-04)

Numbered `900` so it runs **last, deliberately**: a cleanup that runs before the work is finished
deletes scaffolding that was about to be used, and the next tasklist re-adds it.

| Status | Milestone | Tasklist |
|---|---|---|
| ✅ | **Dead code paydown** — the tree is what it claims to be, and the *standard of proof* is the whole tasklist: every removal names the search that found nothing, and [`docs/explanation/dead-code-audit.md`](docs/explanation/dead-code-audit.md) records the candidates that looked dead and were **live** alongside the ones that were not. Measured: 560 production shell functions scanned, exactly **1** with zero references; all 12 duplicate definitions classified as deliberate degradation stubs and **kept**. The rule that did most of the work is not deletion — unreferenced code is removed, but working code no gate invokes is **registered**, because a test in zero gate lists is worse than an absent one: it reads as coverage. Four such were wired into the gate lists, and the sweep also found CI had **not parsed since 2026-08-25** (five step names beginning with a backtick, a reserved YAML indicator — the runs were runs of the file with `jobs=[]`). §9 records what the method could not decide and §10 its limits, in full | `chief/900-dead-code-paydown` · `48cc0ba` |
| ✅ | **The docs tell the truth** — the prose half of the same paydown, against the portfolio's documentation standard, and it ends where a docs sweep usually does not: with what it did NOT verify. One document was superseded and it was the one nothing pointed at — `CHANGELOG.md`, three entries under `## Unreleased`, **0** commits in the 15 merges after 2026-08-21 and **0** gates reading it, while `ROADMAP.md` held the same fact and stayed correct across all 15 because `test/doc-sync.sh` fails CI when it does not. Archived verbatim to `docs/archive/` with the measurement; the root file stays as a pointer, because the standard puts it in Tier 1 where a repo ships. Both ADRs carried `**Status:**` twice (the document's and the question's — one field name, two meanings); the work state is now `**Phase:**`, bodies untouched, each noting that an ADR is superseded and not edited | `chief/901-docs-tell-the-truth` · `61fc39b` |
