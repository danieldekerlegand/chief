...
dep_verdict() {
  ...
  if [ -n "$rr" ] && [ -z "$(resolve_repo "$rr")" ]; then
    printf 'norepo\trepo "%s" is not a path and matches no uniquely-named repo in %s\n' \
      "$rr" "${CHIEF_REPOS:-the known-repos registry}"
    return 0
  fi
  rec="$(dep_record "$d")"
  if [ -n "$rec" ] && [ -f "$rec" ]; then
    printf 'retired\tits record %s has no "mergedToMain" — that record can never satisfy the edge\n' "$rec"
    return 0
  fi
  printf 'unmerged\tno merged record yet (%s)\n' "$rec"
}
