...
| `7` | `paused` | work was **withheld, not broken**: an operator pause (`chief pause`), a Claude usage-limit window, a tasklist blocked `PROVIDER-UNAVAILABLE` (the API never served a request — [provider-unavailability.md](../reference/provider-unavailability.md)), and/or one parked `AWAITING-REVIEW` (a plan nobody has approved — [plan-review.md](../plan-review.md)). Branches and worktrees are kept; re-run to resume |
