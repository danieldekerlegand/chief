...
| `tasklist.incomplete` | `failed` | the iteration budget ran out with stories still unpassed. `detail` opens with WHY the loop gave up — `stalled: N of the last M iteration(s) committed nothing outside .chief/state/ — bookkeeping, not progress`, `stalled: the last M iteration(s) produced no commit at all`, the hard-ceiling case, or the plain budget exhaustion — because "budget ran out" alone covers both a tasklist that was working and ran short and one that stopped advancing on iteration two (see [tasklist-schema.md](tasklist-schema.md)). It then names what the worktree holds UNCOMMITTED when it holds anything (file count, staged/modified/untracked split, and the top-level paths) — the branch is kept, so that work is recoverable rather than lost |
| `tasklist.cannot-complete` | `failed` | the repeat rule fired: the story chief was driving recorded the same outcome — same measurement, same blocker, `passes` unmoved — at `REPEAT_LIMIT` consecutive iteration boundaries, so the tasklist cannot complete as written. **Not a stall**: commits landed on every one of those iterations and all of them are kept, along with the branch and its worktree. `detail` carries the story and the count; the run log and the summary quote the finding and name the two fixes — amend the criterion, or declare the negative terminal (see [tasklist-schema.md](tasklist-schema.md)) |
...
| `tasklist.provider-backoff` | `running` | the bounded **wait** between a transient refusal and the retry — carries `retry_at` (the epoch it wakes at), and `detail` names the delay, the attempt and which rule produced the figure (`retry-after` = the provider named its own interval, `backoff` = capped exponential + jitter). Emitted only for a TRANSIENT refusal: a permanent one (bad or revoked credentials, a quota that will not replenish) is never waited on and goes straight to `tasklist.provider-unavailable` |
