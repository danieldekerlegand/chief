...
if touches '^(bin/chief|engine/.+\.sh|install\.sh|templates/.+\.sh|test/.+\.sh)$'; then
  ...
  if [ "${CHIEF_VERIFY_TESTS:-1}" = "1" ]; then
    for t in smoke ratelimit limitstate limitresume limitmonitor pause liveliness teardown reapscope reapenv noworkguard headless events stale-resume conflict-forensics rebase-refusal touches-audit; do
