...
if touches '^(bin/chief|engine/.+\.sh|install\.sh|templates/.+\.sh|test/.+\.sh)$'; then
  ...
  if [ "${CHIEF_VERIFY_TESTS:-1}" = "1" ]; then
    ...
    CHIEF_BYSTANDER_TESTS="smoke provider-conformance ratelimit limitstate limitresume limitmonitor pause plan-review liveliness teardown reapscope reapenv noworkguard evidence-gate headless events container account-env stale-resume conflict-forensics rebase-refusal touches-audit quality-ratchet gen" \
