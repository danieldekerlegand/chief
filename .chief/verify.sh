...
if touches '^(bin/chief|engine/.+\.sh|install\.sh|templates/.+\.sh|test/.+\.sh)$'; then
  ...
  if [ "${CHIEF_VERIFY_TESTS:-1}" = "1" ]; then
    ...
    CHIEF_BYSTANDER_TESTS="smoke provider-conformance ratelimit provider-unavailable provider-backoff worktree-pending limitstate limitresume limitmonitor pause plan-review research liveliness teardown reapscope reapenv sweep noworkguard evidence-gate criteria-scope measured-bars terminal-false unverified-resume boundary-demotion bookkeeping-progress five-cases stall-flag headless events container account-env stale-resume conflict-forensics rebase-refusal dirty-checkout submodule-gitlink submodule-resume status-deps status-scope status-categories status-json park-reasons touches-audit quality-ratchet overlap-zones merge-batch crossrepo gen" \
