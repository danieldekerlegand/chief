...
if touches '^(bin/chief|engine/.+\.sh|install\.sh|templates/.+\.sh|test/.+\.sh)$'; then
  ...
  if [ "${CHIEF_VERIFY_TESTS:-1}" = "1" ]; then
    ...
    CHIEF_BYSTANDER_TESTS="smoke provider-conformance ratelimit provider-unavailable provider-backoff worktree-pending limitstate limitresume limitmonitor pause plan-review research liveliness teardown reapscope reapenv reap-escaped sweep noworkguard evidence-gate criteria-scope measured-bars terminal-false repeat-stop retire-negative unverified-resume boundary-demotion bookkeeping-progress five-cases stall-flag headless events container account-env stale-resume conflict-forensics rebase-refusal dirty-checkout submodule-gitlink submodule-resume status-deps status-scope status-categories status-json park-reasons parked-decisions decision decision-agent decision-e2e touches-audit quality-ratchet overlap-zones zone-surface zone-friction resolution-deletions resolution-incident merge-batch merge-checkout verify-cache verify-stream cigate crossrepo doc-claims gen update-reroot concurrency machine-budget gate-budget load-admission concurrency-monitor" \
