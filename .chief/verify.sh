...
if touches '^(bin/chief|engine/.+\.sh|install\.sh|templates/.+\.sh|test/.+\.sh)$'; then
  ...
  if [ "${CHIEF_VERIFY_TESTS:-1}" = "1" ]; then
    for t in smoke provider-conformance ratelimit limitstate limitresume limitmonitor pause liveliness teardown reapscope reapenv noworkguard headless events container stale-resume conflict-forensics rebase-refusal touches-audit gen; do
