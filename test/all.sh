...
BASH_SUITE=(
  smoke usage usage-report concurrency machine-budget gate-budget load-admission concurrency-monitor provider provider-conformance ratelimit provider-unavailable provider-backoff worktree-pending limitstate limitresume limitmonitor pause plan-review research liveliness monitor monitor-orphan ps-all teardown reapscope reapenv sweep bystander noworkguard evidence-gate criteria-scope measured-bars terminal-false repeat-stop retire-negative unverified-resume boundary-demotion bookkeeping-progress five-cases stall-flag noworkguard-jsononly headless events container account-env
